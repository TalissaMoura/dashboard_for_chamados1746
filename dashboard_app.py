...
def load_dataframes(ref_date: date):
    ...
    daily_chamados_df = load_daily_data_chamados(
        project_id=PROJ_ID,
        gcp_credentials=st.secrets["GCP_CREDENTIALS"],
        ref_date=ref_date,
    )
...
if "ref_date" not in st.session_state.keys():
    ...
    if last_seven_days_df.shape[0] > 1 or daily_chamados_df.shape[0] > 1:
        with st.container():
            ...
            line_qtd_chamados = (
                alt.Chart(
                    last_seven_days_df.sort_values(by="data_inicio", ascending=True)
                )
                .mark_line(color="gray", point=True)
                .transform_window(
                    window=[
                        {"op": "mean", "field": "qtd_chamados", "as": "media_movel"}
                    ],
                    frame=[None, 0],
                )
                .encode(
        ...
        with st.container():
            ...
            df_qtd_status_chamados = s_qtd_status_chamados.reset_index(
                name="Quantidade"
            )
        ...
        with st.container():
            ...
            df_daily_qtd_tipos_chamados_abertos = (
                df_daily_qtd_tipos_chamados_abertos.join(
            ...
            pie_chart_qtd_tipos_chamados = (
                base_daily_qtd_tipos_chamados_abertos.mark_arc(innerRadius=50)
            )
            st.altair_chart(
        ...
        with st.container():
            ...
            col2.altair_chart(
                bar_qtd_bairros, use_container_width=True, theme="streamlit"
            )
    else:
        with st.container():
            st.text("Não existe dados ainda. Tente novamente mais tarde.")
